...
describe('MoviesPage', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (movieService.getAll as any).mockResolvedValue(mockMovies);
    (movieService.create as any).mockResolvedValue(null);
    (movieService.findByTitle as any).mockResolvedValue([mockMovies[0]]);
  });
  ...
  it('displays error message when create fails', async () => {
    const user = userEvent.setup();
    (movieService.create as any).mockRejectedValue('API Error');
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to create movie: api error/i),
      ).toBeDefined();
    });
  });

  it('handles initial load error', async () => {
    (movieService.getAll as any).mockRejectedValue('Load Error');

    render(<MoviesPage />);

    await waitFor(() => {
      expect(
        screen.getByText(/failed to load movies: load error/i),
      ).toBeDefined();
    });
  });
});
